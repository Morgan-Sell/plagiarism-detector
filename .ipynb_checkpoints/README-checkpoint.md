...
<img src="https://latex.codecogs.com/gif.latex?containment_{(A,S)}&space;=&space;\frac{\sum{count(\text{ngram}_{A})&space;\cap&space;count(\text{ngram}_{S})}}{\sum{count(\text{ngram}_{A})}}" title="containment_{(A,S)} = \frac{\sum{count(\text{ngram}_{A}) \cap count(\text{ngram}_{S})}}{\sum{count(\text{ngram}_{A})}}" />
